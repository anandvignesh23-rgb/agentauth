...
export function buildRiskProfiles(store) {
  ...
  store.data.userRiskProfiles = users.map((user) => {
    ...
    return {
      ...
      mean_amount: Math.round(mean(amounts, 500000)),
      median_amount: Math.round(median(amounts, 500000)),
      max_amount: Math.max(0, ...amounts),
      p95_amount: Math.round(percentile(amounts, 500000)),
...
export function extractTransactionFeatures({ store, agent, merchant, delegation, payload }) {
  ...
  return {
    ...
    user_id: delegation.user_id,
      user_mean_amount: userProfile.mean_amount ?? 500000,
      user_p95_amount: userProfile.p95_amount || 500000,
      agent_mean_amount: agentProfile.mean_amount ?? 500000,
      agent_p95_amount: agentProfile.p95_amount || 500000,
...
export function extractAgentFeatures({ store, agent, delegation, payload }) {
  ...
  return {
    ...
    agent_mean_amount: profile.mean_amount || 500000,
    agent_p95_amount: profile.p95_amount || 500000,
    current_amount_vs_agent_p95: Number(payload.amount) / Math.max(profile.p95_amount || 500000, 1),
