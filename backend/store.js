...
export class PostgresStore {
  ...
  async reload() {
    ...
    this.data = {
      ...
      paymentExecutions: collections[10].rows.map((r) => ({ ...r, authorization_request_id: r.authorization_request_id, created_at: timestamp(r.created_at), paid_at: timestamp(r.paid_at), failed_at: timestamp(r.failed_at), provider_verified_at: timestamp(r.provider_verified_at), amount: Number(r.amount), provider_order: r.provider_order, provider_payment: r.provider_payment })),
      merchantOrders: collections[11].rows.map((r) => ({ ...r, created_at: timestamp(r.created_at), paid_at: timestamp(r.paid_at), amount: Number(r.amount), razorpay_payment_id: r.razorpay_payment_id })),
  ...
  toInsert(collection, r) {
    switch (collection) {
      ...
      case "merchantOrders":
        return buildInsert("merchant_orders", { merchant_id: r.merchant_id, external_order_id: r.external_order_id, description: r.description, amount: r.amount, currency: r.currency, status: r.status, created_at: r.created_at, paid_at: r.paid_at, razorpay_payment_id: r.razorpay_payment_id }, "merchant_id, external_order_id", ["description", "amount", "currency", "status", "paid_at", "razorpay_payment_id"]);
      case "delegations":
